...
async function generateStatementPdf(property: any, payments: any[], expenses: any[], totalIncome: number, totalExpenses: number, netIncome: number, periodLabel: string, month: string): Promise<Uint8Array> {
  ...
  y -= 25;
  page.drawText("Makada Properties", { x: leftMargin, y, font: fontBold, size: 11, color: accent });
  page.drawText(`Period: ${periodLabel}`, { x: 400, y, font, size: 11, color: muted });

  y -= 30;
  page.drawText("Nadeem Makada", { x: leftMargin, y, font: fontBold, size: 10, color: dark });
  y -= 14;
  page.drawText("303 Lakeview Way", { x: leftMargin, y, font, size: 9, color: muted });
  y -= 12;
  page.drawText("Emerald Hills, CA 94062", { x: leftMargin, y, font, size: 9, color: muted });
  ...
  const footerY = 40;
  page.drawText("Generated by Makada Properties", { x: leftMargin, y: footerY, font, size: 8, color: muted });
